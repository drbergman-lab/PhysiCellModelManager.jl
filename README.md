...
- [x] Calibration — ABC-SMC, posterior visualization, and `resumeABC` live in ModelManager. PCMM contributes the PhysiCell-specific measurements: monad-level statistics (`endpointPopulationCounts`, `endpointPopulationFractions`, `meanPopulationTimeSeries`) for analysing a finished monad, and single `Dict`-valued `QoI` builders (`populationCountQoI`, `endpointPopulationFractionQoI`, `meanPopulationTimeSeriesQoI`) to serve as a `CalibrationProblem`'s `summary_statistic`, asserted to agree with the monad-level statistics up to floating-point summation order, including on a monad with a pruned replicate
  ...
  - [x] `populationCountQoI` — the one count builder since `endpointPopulationCountQoI` was removed in 0.5.0 — and `endpointPopulationFractionQoI` serve calibration, the post-processing sink and sensitivity analysis alike. `meanPopulationTimeSeriesQoI` serves calibration alone: every component it reports is a time series rather than the `Real` a sink column or a sensitivity index is computed from
