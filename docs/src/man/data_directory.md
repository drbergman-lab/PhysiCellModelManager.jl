...
After calling [`initializeModelManager`](@ref PhysiCellModelManager.jl), any folder with `base_rulesets.csv` will now be populated with a `base_rulesets.xml` file that can be referenced to set the XML paths.
