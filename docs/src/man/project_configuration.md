...
Upon initialization of the model manager, i.e., calling `using PhysiCellModelManager` (or [`initializeModelManager`](@ref PhysiCellModelManager.jl)), the `inputs.toml` file is parsed and each entry (`config` is shown above) has four features stored:
