...
They must take a single argument, a `Simulation`; declare it `::Simulation`, since an unannotated function cannot be told apart from one written to take a simulation ID (`Int`). A bare function's per-replicate values must average to a `Real`; passed as a [`QoI`](@ref ModelManager.QoI), `reduce` may instead return a `Dict`/`NamedTuple` of `Real`s, which spreads — see below.
