...
function getSimpleContent(xml_doc::XMLDocument, xml_path::Vector{<:AbstractString}; required::Bool=true)
    ...
    @assert elementIsTerminal(e) "Element at path $(join(xml_path, " -> ")) is not a terminal element and has child elements. Cannot get content."
    ret_val = content(e)
    @assert !isempty(ret_val) "Element at path $(join(xml_path, " -> ")) has empty content. Cannot get simple content."
    return ret_val
end
...
"""
function updateSimpleContent(xml_doc::XMLDocument, xml_path::Vector{<:AbstractString}, new_value::Union{Int,Real,String})
...
function createXMLFile(location::Symbol, M::AbstractMonad)
    ...
    if M.variation_id[location] != 0 #! only update if not using the base variation for the location
        ...
        for column_name in names(variation_row)
            ...
            updateSimpleContent(xml_doc, xml_path, variation_row[1, column_name])
        end
    end
...
function configPath(tokens::Vararg{Union{AbstractString,Integer}})
    ...
    elseif length(tokens) == 3
        ...
        elseif token2 == "custom"
            ...
            - `configPath(<cell_type>, "cycle_rate", <start_index>)` (see `cyclePath`)
            - `configPath(<cell_type>, "cycle_duration", <index>)` (see `cyclePath`)
