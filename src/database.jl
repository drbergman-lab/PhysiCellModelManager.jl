export getAllParameterValues, getParameterValue, printSimulationsTable, simulationsTable
...
Recursively traverse the XML element tree to extract parameter values and populate the dictionary `D` with XML paths as keys and their corresponding values.
    ...
    for tag in unique(child_tags)
        ...
        if length(these_children) == 1
            ...
            if !priority_attribute_found
                # no mandatory attribute found, just use the tag
        ...
        for attr in priority_attributes
            if !(attr in common_attributes)
                continue # skip if any child is missing this attribute
            end
        ...
        if isnothing(unique_attribute) # try any common attribute
...
If the string is "true" or "false", it returns a Bool.
If the string can be parsed as a Float64, it returns a Float64.
...
Validate that the `par_key` bytes in the given table match the expected values based on the other columns.
"""
...
function databaseDiagnostics()
    ...
    if !isempty(warning_msg_dirs)
        warning_msg_dirs = "The following have entries in the database but are missing their corresponding folders:\n" * warning_msg_dirs
    ...
    for T in (Monad, Sampling, Trial)
        ...
        @warn "$(lowerClassString(T))s reference non-existent $(lowerClassString(constituentType(T))) IDs: $(sort(collect(missing_ids)))."
    end
    ...
    if !isempty(codes_with_issues)
        arg_string = length(codes_with_issues) == 1 ? codes_with_issues[1] : "[ $(join(codes_with_issues, ", ")) ]"
        @info """
