...
"""
    upgradeToV0_3_0(::Bool)

Upgrade the database to PhysiCellModelManager.jl version 0.3.0.
Adds the `calibrations` table to support the new calibration module.
"""
function upgradeToV0_3_0(auto_upgrade::Bool)
    if !continueMilestoneUpgrade(v"0.3.0", auto_upgrade)
        return false
    end

    ModelManager.createMMTable("calibrations", ModelManager.calibrationsSchema())
    return true
end
