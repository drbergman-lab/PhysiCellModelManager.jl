...
@test_throws AssertionError PhysiCellModelManager.ModelManager.createSchema()
...
@test_throws ErrorException PhysiCellModelManager.createMMTable(table_name_not_end_in_s, "")
schema_without_primary_id = ""
@test_throws ErrorException PhysiCellModelManager.createMMTable("simulations", schema_without_primary_id)
...
PhysiCellModelManager.ModelManager.variationIDs(:config, Simulation(1))
PhysiCellModelManager.ModelManager.variationIDs(:config, Sampling(1))
PhysiCellModelManager.ModelManager.variationIDs(:rulesets_collection, Simulation(1))
PhysiCellModelManager.ModelManager.variationIDs(:rulesets_collection, Sampling(1))
PhysiCellModelManager.ModelManager.variationIDs(:ic_cell, Simulation(1))
PhysiCellModelManager.ModelManager.variationIDs(:ic_cell, Sampling(1))
PhysiCellModelManager.ModelManager.variationIDs(:ic_ecm, Simulation(1))
PhysiCellModelManager.ModelManager.variationIDs(:ic_ecm, Sampling(1))

PhysiCellModelManager.ModelManager.locationVariationsTable(:config, Sampling(1); remove_constants=true)
PhysiCellModelManager.ModelManager.locationVariationsTable(:rulesets_collection, Sampling(1); remove_constants=true)
PhysiCellModelManager.ModelManager.locationVariationsTable(:ic_cell, Sampling(1); remove_constants=true)
PhysiCellModelManager.ModelManager.locationVariationsTable(:ic_ecm, Sampling(1); remove_constants=true)

# test bad folder
path_to_bad_folder = joinpath(PhysiCellModelManager.dataDir(), "inputs", "configs", "bad_folder")
mkdir(path_to_bad_folder)

@test PhysiCellModelManager.ModelManager.reinitializeDatabase() == false

rm(path_to_bad_folder; force=true, recursive=true)
@test PhysiCellModelManager.ModelManager.initializeDatabase() == true
