...
cell_labels = PhysiCellModelManager.cellLabels(out.trial)
substrate_names = PhysiCellModelManager.substrateNames(out.trial)
sequence = PhysiCellSequence(PhysiCellModelManager.trialID(out); include_cells=true, include_substrates=true)

seq_dict = cellDataSequence(sequence, "elapsed_time_in_phase"; include_dead=true)
@test length(seq_dict) == length(seq_dict.dict)
@test seq_dict[PhysiCellModelManager.AgentID(0)] == seq_dict[0]
@test haskey(seq_dict, PhysiCellModelManager.AgentID(0))
@test haskey(seq_dict, 0)
@test_nowarn setindex!(seq_dict, seq_dict[0], PhysiCellModelManager.AgentID(78787878))
@test_nowarn delete!(seq_dict, PhysiCellModelManager.AgentID(78787878))
...
@test PhysiCellModelManager.pathToOutputXML(pruned_simulation) |> PhysiCellModelManager.cellLabels |> isempty
@test PhysiCellModelManager.pathToOutputXML(pruned_simulation) |> PhysiCellModelManager.substrateNames |> isempty
...
PhysiCellModelManager.cellLabels(snapshot)
PhysiCellModelManager.substrateNames(snapshot)
PhysiCellModelManager.loadSubstrates!(sequence)
PhysiCellModelManager.loadMesh!(sequence)
...
mat_pruned_simulation_id = PhysiCellModelManager.trialID(out)
...
@test ismissing(PhysiCellModelManager.averageExtracellularSubstrate(snapshot))
...
PhysiCellModelManager.loadGraph!(sequence, :attachments)
...
txt_pruned_simulation_id = PhysiCellModelManager.trialID(out)
